...
const CarouselWrapper = (props) => {
	...
	return (
		<div className={`carousel--scroll position-relative ${props.amazonLp ? '' : 'px-lg-hg'}`} id={props.id}>
			<div className={`carousel-inner d-flex flex-nowrap row w-auto ${props.className} ${props.amazonLp ? '' : 'px-hg'}`}>
				{props.children}
			</div>
			...
			<a className={`carousel-control-prev text-body ms-ng ${props.withTabs ? 'carousel-control--tabs-prev' : ''}`} role='button'>
            ...
            <a className={`carousel-control-next text-body me-ng ${props.withTabs ? 'carousel-control--tabs-next' : ''}`} role='button'>
